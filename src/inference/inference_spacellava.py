...
def run_infer(args, config: ExperimentConfig):
    # Dynamic imports for SpaceLLaVA (which usually shares code with the LLaVA repository)
    try:
        ...
        logger.error(f"Failed to import SpaceLLaVA libraries. Ensure LLaVA/SpaceLLaVA is installed: {e}")
    ...
    logger.info(f"Using SpaceLLaVA conversation template: {conv_mode}")
