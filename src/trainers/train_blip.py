...
class VQADataset(Dataset):
    """VQA dataset for BLIP-1 training."""
    def __init__(self, data_path: str, image_dir: str, processor: BlipProcessor, max_samples: int = None):
...
@torch.no_grad()
def evaluate_blip(model, dataloader, processor, device, bf16=True):
    model.eval()
    predictions = []
...
def run_train(args, config: ExperimentConfig):
    ...
    for epoch in range(config.training.num_epochs):
        ...
        # Validation
        eval_metrics = evaluate_blip(model, valid_dataloader, processor, device, bf16=config.training.bf16)
