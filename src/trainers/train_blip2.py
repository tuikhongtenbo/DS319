...
class ImageCaptioningDataset(Dataset):
    """Dataset class for BLIP-2 training."""
    def __init__(self, data_path: str, image_dir: str, processor: Blip2Processor, max_samples: int = None):
    ...
    def __getitem__(self, idx: int):
        ...
        labels = self.processor.tokenizer(answer, return_tensors='pt', add_special_tokens=False).input_ids
        
        # Append special EOS token for OPT tokenizer (50118) or default eos_token_id
...
@torch.no_grad()
def evaluate_blip2(model, dataloader, processor, device, bf16=True):
    model.eval()
    predictions = []
    ...
    for batch in tqdm(dataloader, desc="[Valid]"):
        ...
        # Decode targets (ignore padding token or ignore_index 1 / -100)
        pad_token_id = processor.tokenizer.pad_token_id or 1
        for i in range(labels.size(0)):
            valid_label_tokens = labels[i][(labels[i] != pad_token_id) & (labels[i] != -100)]
...
def run_train(args, config: ExperimentConfig):
    ...
    model = Blip2ForConditionalGeneration.from_pretrained(config.model.model_name_or_path, **kwargs)
    ...
    for epoch in range(config.training.num_epochs):
        ...
        for idx, batch in enumerate(pbar):
            ...
            with torch.amp.autocast(device_type="cuda", dtype=dtype):
                # Try standard HuggingFace loss calculation by passing labels
                outputs = model(
                    input_ids=input_ids,
                    pixel_values=pixel_values,
                    attention_mask=attention_mask,
        ...
        # Validation
        eval_metrics = evaluate_blip2(model, valid_dataloader, processor, device, bf16=config.training.bf16)
